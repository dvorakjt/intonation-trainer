...
export class EbClarinet extends TransposingInstrument {
    private voiceNames:string[] = [];

    constructor(L:number, scoreKey:string, partLabel:string) {
        const tranposedKey = transposeKey(scoreKey, 0); //HERE IS WHEN WE START RUNNING INTO PROBLEMS
        super(
            L, 
            Instrument.appendPartLabel("E♭ Clarinet", partLabel), 
            Instrument.appendPartLabel("E♭ Cl.", partLabel), 
            757,
            tranposedKey,
            scoreKey,
            3,
            Instrument.getScoreOrder(INSTRUMENTS_IN_SCORE_ORDER.E_FLAT_CLARINET, partLabel), 
    ...
    addVoice(notes: string, detuneAll:number, detuneChords:detuneChord[]): void {
        const voiceName = "EbClarinet" + (this.partLabel ? this.partLabel : "0") + "V" + this.voices.length;
